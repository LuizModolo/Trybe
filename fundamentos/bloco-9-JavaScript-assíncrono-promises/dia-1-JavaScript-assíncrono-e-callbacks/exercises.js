...
// definição da função sendMarsTemperature...
const sendMarsTemperature = (callbackOne, callbackTwo) => {
  const messageSuccess = Math.random() <= 0.6;
  setTimeout(() => {
    if(messageSuccess) callbackOne(getMarsTemperature());
    else callbackTwo('Robot is busy');
  }, messageDelay());
};
